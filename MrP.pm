...
MrP.pm - Perl package that uses simple majority rule and maximum parsimony
...
sub _traverse {
    ...
    # the start node's as_leaf tag as 1
...
sub find_feature_majority {
    ...
    foreach my $node (@nodes) {
        ...
        # if there are 2 or more majority feature names, look up the majority
        # feature value of the node's ancestor recursively
        if (scalar @majority >= 2) {
...
sub _get_simple_majority_vote {
    ...
    # if the node reaches the root and still does not get a majority feature
    # value, return "NA" as the majority feature value
    if ($node eq $root) {
        return "NA";
    }   
    # diagnostics: print node's ancestor and the majority feature value of
    # the ancestor
    print $fh "\t", $node->get_tag_values("node_number"), "'s ancestor is ", $node->ancestor->get_tag_values("node_number"), " which has the majority feature value ";
