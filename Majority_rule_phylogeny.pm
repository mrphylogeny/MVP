...
Majority_rule_phylogeny.pm - Perl package that uses simple majority rule and
maximum parsimony to determine the majority feature values of all nodes of a
phylogenetic tree, given that a feature is selected.
...
=head1 SYNOPSIS
    ...
    sub option_handler {
        ...
        # Assign the feature selector with the appropriate value
        # according to the value of the option name.
        if ($opt_name eq "date") {
...
The tree is traversed from the root to the leaf nodes. For each node, the
node identifiers of its descendant nodes are collected. This process goes
...
After the procedure, users can obtain text reports and graphic visualization
with the optional utility subroutines for their needs.
...
'accession_number@yyyy/mm/dd@primary_feature@secondary_feature'.

In this format, the feature values are enclosed in single quotation marks,
and @ is the field delimiter, for example, 'AB1234@1997/06/30@Hong_Kong@Human'.

The accession number, date, and primary feature are mandatory, whereas the
secondary feature is optional. The date is in the form of year/month/day
in which the year has 4 digits, month 2 digits, and day 2 digits. The @ sign
is used instead of the underscore as the field delimiter because the
underscore is usually used to separte words, but it cannot be both a field
delimiter and a word separator simultaneously.
...
=item find_feature_majority($tree, $feature_selector)

$tree: input Newick tree

$feature_selector: either DATE, PRIMARY, or SECONDARY (default PRIMARY)
...
=item write_feature_majority($tree, $feature_selector)
...
$majority_threshold: majority feature percentage threshold (default 50)
...
Math::Round and Switch are required. GetOpt::Long is recommended to pass
arguments to subroutines although it is not mandatory.

=head1 AUTHOR

WONG Chi Ho (genewch@yahoo.com)
...
package Majority_rule_phylogeny;

# constants which represent the available values for a feature selector to
# determine what type of feature is selected
use constant PRIMARY => 1;  # primary feature
use constant SECONDARY => 2;    # secondary feature
use constant DATE => 3; # date (year)
...
sub initialize {
    ...
    foreach my $node (@nodes) {
        ...
        $node->add_tag_value("absolute_majority", "none");
        ...
        if ($node->is_Leaf) {
            ...
            # add a default node identifier as node_number@nil
            $node->add_tag_value("collected_node_ids", $node->get_tag_values("node_number") . "\@nil");
        }else {
            $node->id($i);
...
sub _traverse {
    ...
    # flag whether all descendants of $start_node are treated as leaves,
    # initialized as true
    ...
    # check whether all descendants' as_leaf tags are 1; if true, set
    # start node's as_leaf tag as 1
    ALL_ARE_LEAVES: foreach my $node (@descendants) {
        if ($node->get_tag_values("as_leaf") == 0) {
            $all_descendents_as_leaves = 0;
            last ALL_ARE_LEAVES;
        }
        # treat start node as a leaf
        $start_node->set_tag_value("as_leaf", 1);
    }
    # if all descendants of $node are regarded as leaves, add all descendants'
    # features to $node
...
sub _add_node_id {
    ...
    # with a comma to form a comma-delimited string. A delimiter is needed to
    ...
    print $fh "\t", $child->get_tag_values("node_number"), "'s ancestor is ", $current_node->get_tag_values("node_number"), " which has node ids: ", $current_node->get_tag_values("collected_node_ids"), "\n";
...
sub write_collected_node_ids {
    ...
    # write report header
    print $fh "#node_number", "\t", "id", "\t", "type", "\t", "collected_node_ids", "\t", "node_count", "\n";
    foreach my $node (@nodes) {
...
sub find_feature_majority {
    ...
    foreach my $node (@nodes) {
        ...
        # print the feature majority name
        print $fh "\t", "###The feature majority name is ";
        for (my $i = 0; $i < scalar @majority; $i++) {
            print $fh $majority[$i];
            # print the comma if there are 2 or more feature majority names
            # and the current feature is not the last element
            print $fh ", " if (scalar @majority >= 2 && $i != $#majority);
        }
        print $fh "\n";
        
        # if there are 2 or more feature majority names, look up the feature
        # majority value of the node's ancestor recursively
        if (scalar @majority >= 2) {
            my $vote = _get_simple_majority_vote($fh, $node, $root);
            # assign the simple majority vote to the feature majority name
            # of the node
...
sub _get_simple_majority_vote {
    ...
    # if the node reaches the root and still does not get a feature majority
    # value, return "NA" as the feature majority value
    if ($node eq $root) {
        return "NA";
    }   
    # diagnostics: print node's ancestor and the feature majority value of
    # the ancestor
    print $fh "\t", $node->get_tag_values("node_number"), "'s ancestor is ", $node->ancestor->get_tag_values("node_number"), " which has the feature majority value ";
    my @majority = $node->ancestor->get_tag_values("majority");
    for (my $i = 0; $i < scalar @majority; $i++) {
        print $fh $majority[$i];
        # print the comma if there are 2 or more feature majority names and
        # the current feature is not the last element
        print $fh ", " if (scalar @majority >= 2 && $i != $#majority);
    }
    print $fh "\n";
    
    # if $node's ancestor does not have one feature majority, $node's ancestor
    ...
    # if the node reaches the root and the node has finally no feature
    # majority value, return NA as the feature majority value
...
sub _get_feature_values {
    ...
    # The leaf node id is in the form
    # 'accession_number@yyyy/mm/dd@primary_feature@secondary_feature'.
    # The node id is enclosed with single quotes. The field delimiter is @.
    # The primary feature is mandatory, and the secondary feature is optional.
    
    # extract the part within the single quotes
    foreach my $id (@collected_node_ids) {
        # ignore the blank feature values after splitting by comma
        if ($id) {
            $id =~ /\'(\S+)\'/ or die "Error in _get_feature_values(): unmatched node id format: $!.\n";
            my @split_node_id = split /\@/, $1;
            # If the date feature is requested, get the second array element
            # and get the year part; else if the primary feature is requested,
            # get the third array element; else if the secondary feature is
            # requested and the number of features agrees, get the fourth
            # array element. Otherwise add "nil" as a default field value.
            if ($feature_selector == DATE) {
                # split the date by the date separator
                my @split_date = split /\//, $split_node_id[1];
                # return the year part of the date, i.e. the first part
                push @returned_features, $split_date[0];
            } elsif ($feature_selector == PRIMARY) {
                push @returned_features, $split_node_id[2];
            } elsif ($feature_selector == SECONDARY) {
                if (scalar @split_node_id >= 4) {
                    push @returned_features, $split_node_id[3];
                } else {
                    push @returned_features, "nil";
                }
            } else {
                die "Error in _get_feature_values(): unknown feature type or unknown selected feature value to return: $!.\n";
            }
        }
    }
...
sub write_feature_majority {
    ...
    print "Writing feature majority values to $feature_majority.\n";
    ...
    switch ($feature_selector) {
        ...
        case SECONDARY  {print $fh "secondary";}
        case DATE       {print $fh "date";}
        else            {print $fh "unknown feature type";}
    }
    print $fh "\n";
    print $fh "#node_number", "\t", "id", "\t", "ancestor", "\t", "majority", "\t", "count", "\t", "total", "\t", "majority%", "\t", "absolute_majority_(>50%)", "\n";
    
    foreach my $node (@nodes) {
        #The root has no ancestor. Therefore print "NA" as its ancestor.
        print $fh $node->get_tag_values("node_number"), "\t", $node->id, "\t", ($node != $root) ? $node->ancestor->id : "NA", "\t", $node->get_tag_values("majority"), "\t", $node->get_tag_values("hi_count"), "\t", $node->get_tag_values("total_count"), "\t", round($node->get_tag_values("majority_percentage")), "\t", $node->get_tag_values("absolute_majority"), "\n";
    }
    close $fh;
}
...
sub write_feature_summary {
    ...
    my @feature_types = (PRIMARY, SECONDARY, DATE);
    ...
    foreach my $type (@feature_types) {
        ...
        foreach my $leaf (@leaves) {
            push @features, _get_feature_values($leaf, $type);
        }
...
sub find_selected_majority_nodes {
    ...
    # default majority percentage is 50
    $majority_percentage = 50 if (!$majority_percentage);
    # default selected feature type is primary feature
    $feature_selector = PRIMARY if (!$feature_selector);
    ...
    # in which the feature majority matches the target feature values.
    # If the selected feature type is date, further select the nodes in which
    # the feature majority is within the date range.
    ...
    print "Writing selected nodes with feature majority value(s) = @targets and feature majority % >= $majority_percentage to $selected_nodes.\n";
    open my $fh, ">", $selected_nodes or die $!;
    # write report header
    print $fh "#threshold feature majority % = $majority_percentage\n";
    print $fh "#selected feature type = ";
    switch ($feature_selector) {
        case PRIMARY    {print $fh "primary";}
        case SECONDARY  {print $fh "secondary";}
    ...
    print $fh "#node_number", "\t", "id", "\t", "majority", "\t", "majority%", "\t", "descendants", "\n";
...
sub write_output_nexus_tree {
    ...
    # make a copy of the original tree and work on this copy
    ...
    foreach my $node (@nodes) {
        ...
        if ($feature_selector != DATE) {
            TREES_MATCH_TARGETS: foreach my $target (@targets) {
                ...
                if ($node->get_tag_values("majority") =~ /$target/i) {
                    ...
                    # feature majority percentage is still shown, but the
                    # branch is not coloured.
                    $node->set_tag_value("copied_majority_percentage", round($node->get_tag_values("majority_percentage")));
                }
                else {
                    # The feature majority percentage of an unselected branch
                    # is shown as "NA".
        ...
        if ($node->is_Leaf) {
            # For a leaf node, add
            #[&!name="majority_percentage",!color=colour_attribute] to the
            # FigTree_parameters tag.
            ...
            # For an internal node, colour the node in red if its feature
            # majority matches and has a bootstrap value greater than or
    ...
    # append the output NEXUS tree without the #NEXUS header and label
    # translation after the taxa block
